...
async fn main() -> Result<()> {
    env_logger::init();
    println!("Starting Enhanced Rust Sensor (Flow Aggregation + Anomaly Detection)...");
    ...
    let flow_timeout = env::var("FLOW_TIMEOUT_SECONDS")
        .unwrap_or_else(|_| "60".to_string())
        .parse::<u64>()
        .unwrap_or(60);

    let enable_anomaly_detection = env::var("ENABLE_ANOMALY_DETECTION")
    ...
    println!("Flow Timeout: {}s, Anomaly Detection: {}", flow_timeout, enable_anomaly_detection);
    ...
    loop {
        ...
        if flow_export_timer.elapsed().unwrap_or(Duration::ZERO) >= flow_export_interval {
            ...
            for flow in expired_flows {
                ...
                if let Ok(json) = serde_json::to_string(&flow) {
                    let _ = producer.send(
                        FutureRecord::to(kafka_flow_topic)
                            .payload(&json)
                            .key("flow"),
                ...
                if enable_anomaly_detection {
                    let alerts = anomaly_detector.check_flow(&flow);
                    for alert in alerts {
                        if let Ok(json) = serde_json::to_string(&alert) {
                            println!("ALERT: {} - {}", alert.rule_name, alert.description);
                            let _ = producer.send(
                                FutureRecord::to(kafka_alert_topic)
                                    .payload(&json)
                                    .key("alert"),
                                Duration::from_secs(0),
                            );
            ...
            if packet_count % 10000 == 0 {
                println!("Processed {} packets, {} active flows", 
        ...
        match cap.next_packet() {
            Ok(packet) => {
                ...
                // 2. Parse packet and extract flow information
                if let Some(parsed) = parse_packet(packet.data) {
                    if let Some(five_tuple) = parsed.five_tuple {
                        // Update flow tracker
                        flow_tracker.process_packet(
                            five_tuple,
                            parsed.packet_len,
                            parsed.tcp_flags,
                        );
