    ...
    fn rotate_if_needed(&mut self, capture: &Capture<pcap::Active>) -> Result<()> {
        ...
        if should_rotate {
            if let Some(_) = self.current_savefile.take() {
                info!("Rotated pcap file: {}", self.current_file_path);
            }
...
async fn main() -> Result<()> {
    ...
    if let Err(e) = init_telemetry("sensor-service") {
        eprintln!("Failed to initialize telemetry: {}", e);
        std::process::exit(1);
    }

    info!("Starting Enhanced Rust Sensor (Flow Aggregation + Anomaly Detection)...");
                ...
                if enable_anomaly_detection {
                    ...
                    for alert in alerts {
                        if let Ok(json) = serde_json::to_string(&alert) {
                            warn!("ALERT: {} - {}", alert.rule_name, alert.description);
                            let _ = producer.send(
