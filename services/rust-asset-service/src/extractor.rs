...
impl AssetExtractor {
            ...
            .and_then(|v| v.as_str())
        {
                ...
                .and_then(|v| v.as_str())
            {
                ...
                if ua_lower.contains("windows") {
                    asset.os_type = Some("Windows".to_string());
                } else if ua_lower.contains("mac") {
                    asset.os_type = Some("macOS".to_string());
                } else if ua_lower.contains("linux") {
                    asset.os_type = Some("Linux".to_string());
                } else if ua_lower.contains("android") {
                    asset.os_type = Some("Android".to_string());
                } else if ua_lower.contains("ios") || ua_lower.contains("iphone") {
                    asset.os_type = Some("iOS".to_string());
                }
            }
    ...
    fn infer_device_type(port: Option<u64>) -> Option<String> {
        match port {
            Some(p) => {
                let server_ports =
                    vec![80, 443, 22, 21, 25, 110, 143, 3306, 5432, 27017, 6379, 9200];
