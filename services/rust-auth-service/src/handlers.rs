use crate::auth;
use crate::models::{
    ApiKey, ApiKeyResponse, AuthResponse, CreateApiKeyRequest, LoginRequest, RegisterRequest,
};
...
pub async fn register(
    ...
    match state
        .db
        .create_user(&payload.email, &payload.password, &role, &tenant_id)
        .await
    {
...
pub async fn refresh(
    ...
    let token = payload
        ...
        .ok_or((
            ...
            Json(json!({ "error": "Refresh token required" })),
        ))?;
    ...
    let new_token = auth::create_token(
        &claims.sub,
        &claims.role,
        &claims.tenant_id,
        &state.jwt_secret,
...
pub async fn verify(
    ...
    let claims = auth::verify_token(token, &state.jwt_secret).map_err(|e| {
        (
            StatusCode::UNAUTHORIZED,
...
pub async fn create_api_key(
    ...
    let key_hash = auth::hash_password(&key_raw).map_err(|_| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
    ...
    let permissions = payload.permissions.unwrap_or_else(|| {
        vec![
            "logs:create".to_string(),
    ...
    let api_key = state
        .db
        .create_api_key(&payload.name, &key_hash, &permissions, &tenant_id, user_id)
        .await
        .map_err(|e| {
...
pub async fn list_api_keys(
    ...
    let keys = state.db.list_api_keys(user_id).await.map_err(|_| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
...
pub async fn revoke_api_key(
    ...
    state.db.delete_api_key(id, user_id).await.map_err(|_| {
        (
            StatusCode::INTERNAL_SERVER_ERROR,
            Json(json!({ "error": "DB error" })),
        )
...
pub async fn validate_api_key(
    ...
    let key = payload.get("key").and_then(|v| v.as_str()).ok_or((
        StatusCode::BAD_REQUEST,
