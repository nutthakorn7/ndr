...
import { 
  Server, Search, Smartphone, Laptop, 
  Monitor, Printer, Wifi, AlertCircle,
...
export default function AssetDiscovery() {
  ...
  const [stats, setStats] = useState<AssetStats>({ total: 0, new24h: 0, highRisk: 0 });
    ...
    const loadAssets = async () => {
      ...
      try {
        ...
        if (assetsResponse && assetsResponse.assets) {
          ...
          const transformedAssets = assetsResponse.assets.map((asset: any) => ({
            id: asset.id,
          ...
          if (statsResponse) {
            setStats({
              ...
              highRisk: statsResponse.high_risk || transformedAssets.filter((a: Asset) => a.riskScore >= 50).length
            });
          } else {
            setStats({
              total: transformedAssets.length,
              new24h: 0,
              highRisk: transformedAssets.filter((a: Asset) => a.riskScore >= 50).length
            });
          }
        } else {
