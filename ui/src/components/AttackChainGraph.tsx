...
interface GraphNode extends ChainItem {
  x: number;
  y: number;
  type: 'network' | 'file' | 'generic';
}
...
export default function AttackChainGraph({ chain }: AttackChainGraphProps) {
  ...
  useEffect(() => {
    ...
    const newNodes: GraphNode[] = chain.map((alert, index) => ({
      ...alert,
    ...
    const newLinks: GraphLink[] = newNodes.slice(0, -1).map((node, index) => ({
      source: node,
  ...
  const getAlertType = (alert: ChainItem): 'network' | 'file' | 'generic' => {
