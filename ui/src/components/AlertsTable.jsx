...
import { AlertTriangle, Shield, Activity, Download, Play } from 'lucide-react';
...
const AlertsTable = () => {
  ...
  const [requestingPcap, setRequestingPcap] = useState(null);
  ...
  const handleRequestPcap = async (alert) => {
    ...
    try {
      ...
      alert('PCAP request started successfully');
    } catch (error) {
      console.error('Failed to request PCAP:', error);
      alert('Failed to request PCAP');
    } finally {
